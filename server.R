...
server <- function(session, input, output) {
	...
	upload_data <- function(i = "auto") {
		...
		if(input$organism != "Other") {
			maindata$organism <- ensembldb::EnsDb(file.path(getlibfPaths(taxid = 9606, lib = "ORGDB")))
	...
	observeEvent(input$EndStep2, {
		...
		if(is.null(maindata$data_wide_tmp)) {
			...
			if(maindata$isValid) {
				...
				#if(input$organism != "Other") maindata$ <- data.table::fread(file.path(getlibfPaths(maindata$taxid, "STRINGDB")))
